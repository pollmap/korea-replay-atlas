...
export default function PropertyExplorer({atlas,hidden,onClose,onLocate,onViewState}:Props){
  ...
  useEffect(()=>{
    const next={region:regionCode,trade,month,complex:complexId,area,compare,compareComplexes:compareIds,includeReview:showCancelled};onViewState(next);
    ...
    for(const [key,value] of Object.entries({regionCode,trade,month,complex:complexId,area,compareRegions:compare.join(','),compareComplexes:next.compareComplexes.join(','),review:showCancelled?'include':'',propertyRelease:atlas.property.release_id}))if(value)params.set(key,value);else params.delete(key);
  ...
  return <aside className={`property-panel sheet-${sheet}`} hidden={hidden} aria-label="지역·아파트 분석">
    ...
    <p className="property-caption">{region?'현행 지역 코드 조회 결과입니다. 개편 전 코드 대응은 미반영되어 과거 전국 자료의 완전성은 확인되지 않았습니다.':`현행 ${atlas.regions.regions.length.toLocaleString('ko-KR')}개 지역 코드 조회 기준입니다. 개편 전 코드 대응은 미반영 상태입니다.`}</p>
    ...
    {!region?<>
      <div className="property-stat"><span>{collected===atlas.regions.regions.length?'전국':'수집 완료 지역'} 거래량</span><strong>{collected?total.toLocaleString('ko-KR'):'—'}<small>건</small></strong><span>{collected} / {atlas.regions.regions.length}개 지역 확인</span></div>
      ...
      {busy?<p role="status">지역 자료를 확인하고 있습니다…</p>:selectedDetail&&<>
        <div className="property-select-row"><label>계약월<select value={month} onChange={e=>setMonth(e.target.value)}>{[...new Set(months)].reverse().map(m=><option value={m} key={m}>{monthLabel(m)}{m===selectedDetail.period.to?' · 잠정':''}</option>)}</select></label><button className="compare-region" aria-pressed={compare.includes(regionCode)} disabled={compare.length>=3&&!compare.includes(regionCode)} onClick={()=>toggleCompare(regionCode)}>지역 비교 {compare.includes(regionCode)?'✓':'+'}</button></div>
        <div className="property-stat"><span>{complexId?'선택 단지·면적 신고 거래량':'지역 전체 신고 거래량'}</span><strong>{current&&metricCount(current)!==null?(complexId?(rowsView.state!=='ready'||!complex?'—':transactionRows(rows,{trade,complex:complexId,area,cancelled:false}).length.toLocaleString('ko-KR')):metricCount(current)!.toLocaleString('ko-KR')):'—'}<small>건</small></strong><span>{current?propertyStatus(current.status):'자료 없음'} · {trade==='sale'?'취소·취소 여부 미확인 제외':'원천에서 취소 여부 미제공'}</span></div>
