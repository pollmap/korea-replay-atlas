...
import {regionNavigation,regionNavigationPlace} from './region-navigation';
...
interface Props {mapLayout?:boolean;atlas:AtlasContent;hidden:boolean;onClose:()=>void;onLocate:(place:Place)=>void;onViewState:(state:PropertyViewState)=>void;onMapPoint?:(point:PropertyMapPoint|null)=>void;requestedRegion?:{code:string;request:number;complexId?:string;skipLocate?:boolean};}
...
export default function PropertyExplorer({mapLayout=false,atlas,hidden,onClose,onLocate,onViewState,onMapPoint,requestedRegion}:Props){
  ...
  const watchlist=usePropertyWatchlist();
  const [dashboardTab,setDashboardTab]=useState<'explore'|'watchlist'>('explore');
  const watchedIds=useMemo(()=>new Set(watchlist.items.map(row=>row.id)),[watchlist.items]);
  const watchComplex=(value:PropertyComplex)=>watchlist.toggle({id:value.id,regionCode:value.lawd_code,name:value.name,address:[value.legal_dong_name,value.lot_number].filter(Boolean).join(' '),savedAt:new Date().toISOString()});
  const openWatched=(item:WatchedComplex)=>{const target=atlas.regions.regions.find(row=>row.lawd_code===item.regionCode);if(!target){watchlist.setMessage('이 자료 버전에 해당 지역이 없습니다. 저장한 단지는 그대로 유지됩니다.');return;}setDashboardTab('explore');setPropertyType('apartment');setRegionCode(item.regionCode);setComplexId(item.id);setArea('');setMapPointCandidate(false);locateAfterSelection.current=item.id;const place=regionNavigationPlace(target,atlas.map);if(place)onLocate(place);};
  ...
  const [detailNavigation,setDetailNavigation]=useState<{id:string;section:'trades'|'info'|'compare'|'commute'}>({id:'',section:'trades'});
  ...
  useEffect(()=>{
    ...
    setDashboardTab('explore');setPropertyType('apartment');
  ...
  const provinces=[...new Set(atlas.regions.regions.map(row=>row.name.trim().split(/\s+/)[0]))];
  const selectedProvince=region?.name.trim().split(/\s+/)[0]??(provinces.includes(filter)?filter:'');
  const chooseProvince=(name:string)=>{setFilter(name);setRegionCode('');setComplexId('');setArea('');setMapPointCandidate(false);setDashboardTab('explore');resetScroll();const target=name?regionNavigation({name},atlas.map)?.place:PLACES[0];if(target)onLocate(target);};
  return <aside className={`property-panel sheet-${sheet} ${complex?'showing-complex':region?'showing-region':'showing-country'}`} data-trade={trade} hidden={hidden} aria-label="지역·주거 분석">
    ...
    {mapLayout&&!complex&&propertyType==='apartment'&&dashboardTab==='explore'&&<nav className="property-region-picker" aria-label="지역 단계 선택"><label><span className="sr-only">시도</span><select aria-label="시도 선택" value={selectedProvince} onChange={event=>chooseProvince(event.target.value)}><option value="">전국</option>{provinces.map(name=><option key={name} value={name}>{name}</option>)}</select></label><span aria-hidden="true">›</span><label><span className="sr-only">시군구</span><select aria-label="시군구 선택" value={regionCode} disabled={!selectedProvince} onChange={event=>{const row=atlas.regions.regions.find(row=>row.lawd_code===event.target.value);if(row)chooseRegion(row);else chooseProvince(selectedProvince);}}><option value="">시·군·구 전체</option>{atlas.regions.regions.filter(row=>row.name.trim().split(/\s+/)[0]===selectedProvince).map(row=><option value={row.lawd_code} key={row.lawd_code}>{row.name.replace(selectedProvince,'').trim()||row.name}</option>)}</select></label></nav>}
    <header className="property-header"><div>{region?<button className="property-back" onClick={()=>{setMapPointCandidate(false);if(complexId){setComplexId('');setArea('');}else{setRegionCode('');onLocate(PLACES[0]);}resetScroll();}}>{complexId?`← ${region.name}`:'← 전국 지역 목록'}</button>:<span className="eyebrow">전국 주거 실거래 분석</span>}<h2 ref={heading} tabIndex={-1}>{dashboardTab==='watchlist'?'나의 관심 아파트':complex?.name??region?.name??(mapLayout?'전국 아파트':'어느 지역을 살펴볼까요?')}</h2>{!complex&&dashboardTab==='explore'&&<div className="property-type-switch" role="group" aria-label="주거 유형">{([['apartment','아파트'],['officetel','오피스텔']] as const).map(([type,label])=><button key={type} aria-pressed={propertyType===type} onClick={()=>{if(propertyType===type)return;setPropertyType(type);setRegionCode('');setComplexId('');setArea('');setCompare([]);setCompareIds([]);setCompareComplexes([]);setMapPointCandidate(false);setFilter('');setError('');onLocate(PLACES[0]);resetScroll();}}>{label}</button>)}</div>}{dashboardTab==='explore'&&complex&&<p className="complex-subtitle">{[complex.legal_dong_name,complex.lot_number].filter(Boolean).join(' ')}{complex.build_year!==null?` · ${complex.build_year}년 건축`:''}</p>}</div><button aria-label="지역 분석 닫기" onClick={onClose}>×</button></header>
    {propertyType==='apartment'&&<nav className="dashboard-nav" aria-label="아파트 대시보드"><button aria-pressed={dashboardTab==='explore'} onClick={()=>setDashboardTab('explore')}>아파트 탐색</button><button aria-pressed={dashboardTab==='watchlist'} onClick={()=>setDashboardTab('watchlist')}>관심 단지 <span>{watchlist.items.length}</span></button></nav>}
    {watchlist.message&&<p className="watchlist-message" role="status">{watchlist.message}</p>}
    {dashboardTab==='explore'&&complex&&<div className="complex-actionbar" role="group" aria-label="선택 단지 도구"><button className="complex-watch" aria-pressed={watchedIds.has(complex.id)} onClick={()=>watchComplex(complex)}>{watchedIds.has(complex.id)?'★ 관심 단지':'☆ 관심 저장'}</button><button disabled={compareIds.length>=3&&!compareIds.includes(complex.id)} aria-pressed={compareIds.includes(complex.id)} onClick={()=>setCompareIds(previous=>previous.includes(complex.id)?previous.filter(id=>id!==complex.id):[...previous,complex.id])}>{compareIds.includes(complex.id)?'비교 담음':'비교 담기'}</button>{mapPoint?<button title="서울시 제공 점 · 위치 검증 중" onClick={()=>onLocate({id:complex.id,name:complex.name,region:region?.name??'',lon:mapPoint.longitude,lat:mapPoint.latitude,range:1300})}>지도 위치</button>:complex.position?<button onClick={()=>onLocate({id:complex.id,name:complex.name,region:region?.name??'',lon:complex.position!.longitude,lat:complex.position!.latitude,range:1300})}>지도 위치</button>:null}</div>}
    {dashboardTab==='explore'&&complex&&<nav className="complex-section-nav" aria-label="단지 상세 메뉴">{([['trades','실거래'],['info','단지정보'],['commute','출퇴근'],['compare',`비교${compareIds.length?` ${compareIds.length}`:''}`]] as const).map(([section,label])=><button key={section} aria-pressed={detailSection===section} onClick={()=>{setDetailNavigation({id:complexId,section});resetScroll();}}>{label}</button>)}</nav>}
    <div className="property-body">


    {propertyType==='apartment'&&dashboardTab==='watchlist'?<PropertyWatchlist items={watchlist.items} onOpen={openWatched} onRemove={watchlist.remove} onImport={watchlist.importItems} onMessage={watchlist.setMessage}/>:propertyType==='officetel'?<section className="property-data-pending" aria-live="polite"><h3>오피스텔 자료 준비 중</h3><p>검증된 매매·전월세 자료가 게시되면 제공됩니다.</p><button onClick={()=>setPropertyType('apartment')}>아파트 보기</button></section>:<>
    {(!complex||detailSection==='trades'||detailSection==='compare')&&<div className="property-querybar"><div className="property-trade" role="group" aria-label="거래 유형"><button aria-pressed={trade==='sale'} onClick={()=>{setTrade('sale');setArea('');}}>매매</button><button aria-pressed={trade==='rent'} onClick={()=>{setTrade('rent');setArea('');}}>전월세</button></div>{region&&selectedDetail?<label className="property-month"><span className="sr-only">계약월</span><select aria-label="계약월" value={month} onChange={e=>setMonth(e.target.value)}>{[...new Set(months)].reverse().map(m=><option value={m} key={m}>{monthLabel(m)}{m===selectedDetail.period.to?' · 잠정':''}</option>)}</select></label>:<span className="property-period">{monthLabel(atlas.property.period.latest_complete_month)} 계약</span>}</div>}
    {region&&(!complex||detailSection==='trades')&&month===atlas.property.period.to&&<p className="property-caption">당월 잠정 자료 · 추가 신고에 따라 바뀔 수 있습니다.</p>}
    ...
    {!region?<>
      ...
      {busy?<div className="property-loading" role="status"><p>지역 자료를 확인하고 있습니다…</p><i/><i/><i/></div>:selectedDetail&&<>
        ...
        <div hidden={!!complexId}><PropertyComplexList key={mapLayout?`${regionCode}:${trade}`:regionCode} savedFilterRegion={mapLayout?regionCode:undefined} watchedIds={watchedIds} onWatch={watchComplex} complexes={complexes} rows={rowsKey===queryKey?rows:[]} dataReady={rowsView.state==='ready'} trade={trade} onSelect={id=>{locateAfterSelection.current=id;setComplexId(id);setMapPointCandidate(false);setArea('');heading.current?.focus();const body=heading.current?.closest('aside')?.querySelector('.property-body');if(body)body.scrollTop=0;}}/></div>
        ...
        {complex&&detailSection==='commute'&&<PropertyCommute complex={complex} region={region.name}/>}
        {complex&&<section hidden={detailSection!=='info'} className="complex-facts compact complex-information" aria-label="공식 단지 정보"><h3>단지 기본정보</h3>{mapLayout&&detailSection==='info'&&<ApartmentFacts complexId={complex.id} release={atlas.property.release_id}/>}<dl><div><dt>단지명</dt><dd>{complex.name}</dd></div><div><dt>지역</dt><dd>{region.name}</dd></div><div><dt>법정동·지번</dt><dd>{[complex.legal_dong_name,complex.lot_number].filter(Boolean).join(' ')||'원문 미제공'}</dd></div><div><dt>건축연도</dt><dd>{complex.build_year??'원문 미제공'}</dd></div><div><dt>공식 식별번호</dt><dd>{complex.source_complex_id}</dd></div></dl><p className="property-caption">국토교통부 실거래 신고 원문 기준입니다. 건축연도는 입주 예정일과 구분합니다.</p>{pointError&&<p className="property-caption" role="status">{pointError} <button onClick={()=>setPointAttempt(value=>value+1)}>위치 다시 불러오기</button></p>}{complex.position?<button onClick={()=>onLocate({id:complex.id,name:complex.name,region:region.name,lon:complex.position!.longitude,lat:complex.position!.latitude,range:1300})}>지도에서 보기 ↗</button>:<details className={providerPointLinked?'property-coordinate-note':undefined}><summary>{providerPointLinked?'서울시 제공 점 · 좌표 정확도 검토 중':'지역 지도 표시 중 · 단지 좌표 미연결'}</summary><p>{providerPointLinked?'서울시 단지 점과 국토부 실거래 ID를 유일한 도로명주소·단지명으로 연결했습니다. 지도 점의 좌표계와 위치 의미는 아직 검증 중이며, 실거래 단지의 확정 좌표로 취급하지 않습니다.':'공식 좌표가 확인되지 않아 단지 위치를 임의로 표시하지 않습니다.'}</p></details>}</section>}
