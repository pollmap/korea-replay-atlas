...
export default function PropertyExplorer({atlas,hidden,onClose,onLocate,onViewState,requestedRegion}:Props){
  ...
  useEffect(()=>{
    const next={region:regionCode,trade,month,complex:complexId,area,compare,compareComplexes:compareIds,includeReview:showCancelled,historyMonths:historyRange};onViewState(next);
    ...
    for(const [key,value] of Object.entries({regionCode,trade,month,complex:complexId,area,compareRegions:compare.join(','),compareComplexes:next.compareComplexes.join(','),review:showCancelled?'include':'',historyMonths:String(historyRange),propertyRelease:atlas.property.release_id}))if(value)params.set(key,value);else params.delete(key);
  ...
  return <aside className={`property-panel sheet-${sheet} ${complex?'showing-complex':region?'showing-region':'showing-country'}`} hidden={hidden} aria-label="지역·아파트 분석">
    ...
    {!region?<>
      ...
      {busy?<div className="property-loading" role="status"><p>지역 자료를 확인하고 있습니다…</p><i/><i/><i/></div>:selectedDetail&&<>
        ...
        {complex&&<div className="complex-facts compact"><p>{[complex.legal_dong_name,complex.lot_number].filter(Boolean).join(' ')} <span>· {complex.build_year===null?'건축연도 미제공':`${complex.build_year}년 건축`}</span></p>{complex.position?<button onClick={()=>onLocate({id:complex.id,name:complex.name,region:region.name,lon:complex.position!.longitude,lat:complex.position!.latitude,range:1300})}>지도에서 보기 ↗</button>:<details className={mapPointCandidate?'property-coordinate-note':undefined}><summary>{mapPointCandidate?'서울시 제공 점 · 좌표 정확도 검토 중':'지역 지도 표시 중 · 단지 좌표 미연결'}</summary><p>{mapPointCandidate?'서울시 단지 점과 국토부 실거래 ID를 유일한 도로명주소·단지명으로 연결했습니다. 지도 점의 좌표계와 위치 의미는 아직 검증 중이며, 실거래 단지의 확정 좌표로 취급하지 않습니다.':'공식 좌표가 확인되지 않아 단지 위치를 임의로 표시하지 않습니다.'}</p></details>}</div>}
        <div className="property-comparison-action">{complex?<button className="compare-region" disabled={compareIds.length>=3&&!compareIds.includes(complex.id)} aria-pressed={compareIds.includes(complex.id)} onClick={()=>setCompareIds(previous=>previous.includes(complex.id)?previous.filter(id=>id!==complex.id):[...previous,complex.id])}>{compareIds.includes(complex.id)?'비교에 담음 ✓':'단지 비교에 담기 +'}</button>:<button className="compare-region" aria-pressed={compare.includes(regionCode)} disabled={compare.length>=3&&!compare.includes(regionCode)} onClick={()=>toggleCompare(regionCode)}>지역 비교 {compare.includes(regionCode)?'✓':'+'}</button>}</div>
