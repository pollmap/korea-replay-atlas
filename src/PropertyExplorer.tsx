  ...
  return <aside className={`property-panel sheet-${sheet} ${complex?'showing-complex':region?'showing-region':'showing-country'}`} data-trade={trade} hidden={hidden} aria-label="지역·주거 분석">
    ...
    {!region?<>
      ...
      {busy?<div className="property-loading" role="status"><p>지역 자료를 확인하고 있습니다…</p><i/><i/><i/></div>:selectedDetail&&<>
        ...
        <div hidden={!!complexId}><PropertyComplexList key={regionCode} watchedIds={watchedIds} onWatch={watchComplex} complexes={complexes} rows={rowsKey===queryKey?rows:[]} dataReady={rowsView.state==='ready'} trade={trade} onSelect={id=>{locateAfterSelection.current=id;setComplexId(id);setMapPointCandidate(false);setArea('');heading.current?.focus();const body=heading.current?.closest('aside')?.querySelector('.property-body');if(body)body.scrollTop=0;}}/></div>
        ...
        {complex&&<section hidden={detailSection!=='info'} className="complex-facts compact complex-information" aria-label="공식 단지 정보"><h3>단지 기본정보</h3><dl><div><dt>단지명</dt><dd>{complex.name}</dd></div><div><dt>지역</dt><dd>{region.name}</dd></div><div><dt>법정동·지번</dt><dd>{[complex.legal_dong_name,complex.lot_number].filter(Boolean).join(' ')||'원문 미제공'}</dd></div><div><dt>건축연도</dt><dd>{complex.build_year??'원문 미제공'}</dd></div><div><dt>공식 식별번호</dt><dd>{complex.source_complex_id}</dd></div></dl><p className="property-caption">국토교통부 실거래 신고 원문 기준입니다. 건축연도는 입주 예정일과 구분합니다.</p>{pointError&&<p className="property-caption" role="status">{pointError} <button onClick={()=>setPointAttempt(value=>value+1)}>위치 다시 불러오기</button></p>}{complex.position?<button onClick={()=>onLocate({id:complex.id,name:complex.name,region:region.name,lon:complex.position!.longitude,lat:complex.position!.latitude,range:1300})}>지도에서 보기 ↗</button>:<details className={providerPointLinked?'property-coordinate-note':undefined}><summary>{providerPointLinked?'서울시 제공 점 · 좌표 정확도 검토 중':'지역 지도 표시 중 · 단지 좌표 미연결'}</summary><p>{providerPointLinked?'서울시 단지 점과 국토부 실거래 ID를 유일한 도로명주소·단지명으로 연결했습니다. 지도 점의 좌표계와 위치 의미는 아직 검증 중이며, 실거래 단지의 확정 좌표로 취급하지 않습니다.':'공식 좌표가 확인되지 않아 단지 위치를 임의로 표시하지 않습니다.'}</p></details>}</section>}
