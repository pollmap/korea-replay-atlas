...
export function vectorLayers(topic:MapTileTopic,source:string):LayerSpecification[]{
  const kind=topic.id==='detail-roads'?'roads':topic.id;
  ...
  if(['land','water','buildings','facilities'].includes(kind))rows.push({...base,id:`${id}-fill`,type:'fill',filter:['==',['geometry-type'],'Polygon'],paint:{'fill-color':kind==='water'?'#c9e3f3':kind==='buildings'?'#dce3ec':kind==='land'?'#f8fafc':'#e5f0e8','fill-outline-color':kind==='buildings'?'#c9d3df':kind==='water'?'#c9e3f3':'#e0e8ed','fill-opacity':.95}});
  if(['roads','rail','water'].includes(kind))rows.push({...base,id:`${id}-line`,type:'line',filter:['==',['geometry-type'],'LineString'],layout:{'line-join':'round','line-cap':'round'},paint:{'line-color':kind==='rail'?'#97857d':kind==='water'?'#b3d4eb':['match',['coalesce',['get','highway'],['get','class'],'other'],['motorway','motorway_link'],'#d89577',['trunk','trunk_link'],'#e2b875',['primary','primary_link'],'#e7ce89','#fffdf4'],'line-width':['interpolate',['linear'],['zoom'],4,.5,9,1.2,13,2.6,17,6],'line-opacity':kind==='rail'?.8:1}});
  rows.push({...base,id:`${id}-point`,type:'circle',minzoom:8,filter:['==',['geometry-type'],'Point'],paint:{'circle-radius':['interpolate',['linear'],['zoom'],8,1.2,16,3],'circle-color':kind==='water'?'#b3d4eb':'#91a7b6'}});
