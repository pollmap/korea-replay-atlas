...
export function useAtlas(runtime:RuntimeV2|null,checked:boolean,runtimeError=''){
  ...
  useEffect(()=>{
    ...
    void(async()=>{
      ...
      if(requested&&requested!==property.release_id){
        const archiveUrl=propertyReleaseArchiveUrl(location.href,property.release_id);
        if(archiveUrl){
          if(controller.signal.aborted)return;
          setResult({state:'loading',content:null,error:''});
          location.replace(archiveUrl);
          return;
        }
        throw new Error('공유된 부동산 자료 버전이 이 배포에 없습니다. 최신 자료로 대체하지 않습니다.');
      }
