...
function ToolIcon({kind}:{kind:'location'|'bookmark'|'tools'}){
...
export default function MapInteractionTools({map,view,place,hidden,measurement,onMeasure,onUndo,onNotice,onLocate}:Props){
  ...
  useEffect(()=>{
    ...
    const close=(event:PointerEvent)=>{if(event.target instanceof Node&&!toolsRef.current?.contains(event.target)){setOpen(false);setToolsOpen(false);}};
    const escape=(event:KeyboardEvent)=>{if(event.key==='Escape'){toolsRef.current?.querySelector<HTMLButtonElement>('.interaction-buttons button[aria-expanded="true"]')?.focus();setOpen(false);setToolsOpen(false);}};
  ...
  return <div className="interaction-tools" ref={toolsRef} hidden={hidden}>
    <div className="interaction-buttons" role="group" aria-label="지도 도구">
      ...
      <button className="interaction-bookmarks-toggle" aria-label={`즐겨찾기${bookmarks.length?` ${bookmarks.length}개`:''}`} aria-expanded={open} aria-controls="map-bookmarks" onClick={()=>{setOpen(value=>!value);setToolsOpen(false);}} title="저장한 위치"><ToolIcon kind="bookmark"/><span>즐겨찾기</span></button>
      <button aria-label="측정·저장 도구" aria-expanded={toolsOpen} aria-controls="map-extra-tools" onClick={()=>{setToolsOpen(value=>!value);setOpen(false);}} title="측정·저장 도구"><ToolIcon kind="tools"/><span>도구</span></button>
    </div>
