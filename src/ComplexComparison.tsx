...
export default function ComplexComparison({atlas,items,month,trade,area,onArea,onRemove,rentKind='all'}:{atlas:AtlasContent;items:PropertyComplex[];month:string;trade:'sale'|'rent';area:string;onArea:(area:string)=>void;onRemove:(id:string)=>void;rentKind?:RentKind}){
  ...
  const areas=useMemo(()=>rowsKey===queryKey?[...new Set(rows.filter(row=>row.trade_type===trade&&rentKindMatches(row,rentKind)&&ids.has(row.complex_id??'')&&row.area_m2).map(row=>row.area_m2!))].sort((a,b)=>Number(a)-Number(b)):[],[rows,rowsKey,queryKey,ids,trade,rentKind]);
  const filteredRows=useMemo(()=>rows.filter(row=>rentKindMatches(row,rentKind)),[rows,rentKind]);
  ...
  return <section className="complex-comparison" aria-label="아파트 단지 비교"><h3>단지 비교 <small>{items.length} / 3</small></h3><p className="property-caption">{monthLabel(month)} · {trade==='sale'?'매매':rentKindLabel(rentKind)} · 같은 전용면적 조건 · 84㎡대 선택 가능</p>
    ...
    {waiting?<p role="status">비교 자료를 불러오는 중…</p>:error?<><p role="alert">{error}</p><button onClick={()=>setAttempt(value=>value+1)}>비교 자료 다시 불러오기</button></>:<div className="transaction-table"><table><thead><tr><th>단지</th><th>표본</th><th>{trade==='sale'?'거래 중앙값':'보증금 / 월세 중앙값'}</th></tr></thead><tbody>{items.map(item=>{
      const summary=area&&!missing.includes(item.lawd_code)?summarizePropertyTransactions(filteredRows,{complex_id:item.id,trade_type:trade,area_m2:area,from:start,to:end}):null;
