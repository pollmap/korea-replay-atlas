...
const Map2D=forwardRef<MapHandle,Props>(function Map2D(props,ref){
  ...
  useEffect(()=>{
    ...
    const loadSeoulKaptPoints=()=>{
      ...
      map.addLayer({id:'seoul-kapt-linked-sale',type:'symbol',source:SEOUL_KAPT_SOURCE,minzoom:14,filter:['all',['!',['has','point_count']],['has','recent_sale_label']],layout:{'text-field':['concat',['get','name'],'\n',['get','recent_sale_label']],'text-font':['Malgun Gothic','sans-serif'],'text-size':11,'text-offset':[0,1.35],'text-anchor':'top','text-max-width':16,'text-optional':true},paint:{'text-color':'#174c38','text-halo-color':'#fff','text-halo-width':2}});
