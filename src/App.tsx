...
import {isApartmentMapSelection} from '../shared/selection';
...
export default function App(){
  ...
  const selectPropertyProvince=useCallback((province:string)=>{setMode('map');setPlaying(false);setTimeOpen(false);setSunOpen(false);setCityToolsOpen(false);setPropertyOpen(true);setSelection(null);setMenuOpen(false);setFocusMode(false);setRequestedRegion(previous=>({code:'',province,request:(previous?.request??0)+1}));},[]);
  const [propertyRegion,setPropertyRegion]=useState(()=>new URLSearchParams(location.hash.slice(1)).get('regionCode')??'');
  const onPropertyView=useCallback((value:PropertyViewState)=>{propertyViewRef.current=value;setPropertyTrade(value.trade);setPropertyRegion(value.region);},[]);
  ...
  const propertyRequested=mapView==='2d'&&!focusMode&&mode==='map'&&!menuOpen&&!selection&&propertyOpen;
  ...
  return <div className={`app-shell map-first atlas-shell is-${mapView} ${propertyVisible?'has-property':propertyRequested&&atlas.state==='loading'?'reserves-property':''} ${mode==='map'?'is-exploring':''} ${mode==='live'?'is-live':''} ${focusMode?'is-focused':''} ${menuOpen?'has-layers':''} ${sunOpen?'has-sun':''} ${selection?'has-selection':''} ${mode==='live'&&observationsOpen?'has-observations':''}`}>
    ...
    <nav className="map-dock" aria-label="지도 패널" hidden={focusMode||(mapView==='2d'&&mode==='map')}>
      ...
      {atlas.content&&<button onClick={()=>{if(mode!=='map')switchMode('map');setPropertyOpen(mode==='map'?!propertyVisible:true);setMenuOpen(false);setSelection(null);}} aria-expanded={propertyVisible}>목록 보기</button>}
    ...
    {mapView==='2d'&&mode==='map'&&!propertyVisible&&!focusMode&&<button className="property-reopen" onClick={()=>{setPropertyOpen(true);setMenuOpen(false);setSelection(null);}}>아파트 목록</button>}
    ...
    {atlas.content&&<Suspense fallback={null}><PropertyExplorer mapLayout={mapView==='2d'&&mode==='map'} key={atlas.content.property.release_id} atlas={atlas.content} hidden={!propertyVisible} onClose={()=>setPropertyOpen(false)} onLocate={goTo} onViewState={onPropertyView} onMapRegionName={setPropertyRegionName} onMapPoint={setPropertyMapPoint} requestedRegion={requestedRegion}/></Suspense>}
