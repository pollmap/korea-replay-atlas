...
후속 수집·게시 회귀를 합친 집중 검증은 Python 73개·TypeScript 38개가 통과했습니다. 인증 실패와 정상 빈 응답, 호출 예약의 재시작 보존, 반사된 키의 저장 거부, 원본 변경 감지, 단일 수집 임대와 만료 소유자의 상태 덮어쓰기 거부, 새 변환의 원본 재처리, 월 필터 밖 상태 보존, 파일 기록 중단 시 불완전 최종 파일 미노출과 지번/가격 품질 분리를 포함합니다. 공용 TS의 독립 타입 검사와 린트도 통과했습니다. 실제 후보의 모든 파일에 대한 재파싱·TS 디코딩·SHA·partition 건수 결과는 운영 문서의 불변 릴리스별 증빙으로 확인합니다.
