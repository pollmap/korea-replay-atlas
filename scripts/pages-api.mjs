...
export function createPagesApi({accountId,token,fetcher=fetch}){
  ...
  async function request(route,{method='GET',body,uploadToken}={}){
    ...
    let payload;try{payload=await response.json();}catch{throw new Error(`Pages API invalid response (${response.status})`);}
    if(!response.ok||payload?.success!==true){const codes=(payload?.errors??[]).map(error=>Number(error.code)).filter(Number.isFinite);throw new Error(`Pages API rejected request (${response.status}; codes ${codes.join(',')||'unknown'})`);}
...
export async function deployPagesStage({receiptPath,projectRoot=process.cwd(),api,production=false,onProgress=()=>{}}){
  ...
  const jwt=await api.uploadToken(project),hashes=[...new Set([...map.values()].map(entry=>entry.key))],missing=await api.missing(hashes,jwt);
  ...
  await Promise.all(Array.from({length:2},async()=>{while(cursor<buckets.length){const current=buckets[cursor++],items=[];
    ...
    await api.upload(items,jwt);uploaded+=current.length;onProgress({phase:'upload',files:uploaded,total:unique.size});
  }}));
  await api.retain(hashes,jwt);
...
export async function verifyPagesRemote({receiptPath,origin,projectRoot=process.cwd(),fetcher=fetch}){
  ...
  for(const name of names){const entry=checked.entries.find(value=>value.target===name);if(!entry)throw new Error('Required remote sample missing from stage');const response=await get('/'+name),bytes=Buffer.from(await response.arrayBuffer());
    if(response.status!==200||bytes.length!==entry.bytes||sha(bytes)!==entry.sha256)throw new Error('Remote static bytes differ: '+name);
    if(project==='korea-replay-data'&&response.headers.get('Access-Control-Allow-Origin')!=='*')throw new Error('Data CORS is not public');samples.push({path:'/'+name,passed:true});}
